function compareTires_FySlip_GUI_4()
%COMPARETIRES_FYSLIP_GUI Compare Fy vs slip angle for four TIR files using PAC2002.
...
%   remains similar and asks for four TIR files at startup. An additional
...
[file3,path3] = uigetfile('*.tir','Seleziona il terzo file .tir'); if isequal(file3,0), return; end
[file4,path4] = uigetfile('*.tir','Seleziona il quarto file .tir'); if isequal(file4,0), return; end

p1 = readTirParameters(fullfile(path1,file1));
p2 = readTirParameters(fullfile(path2,file2));
p3 = readTirParameters(fullfile(path3,file3));
p4 = readTirParameters(fullfile(path4,file4));
[~,name1] = fileparts(file1); tireName1 = getParam(p1,'tireName',name1);
[~,name2] = fileparts(file2); tireName2 = getParam(p2,'tireName',name2);
[~,name3] = fileparts(file3); tireName3 = getParam(p3,'tireName',name3);
[~,name4] = fileparts(file4); tireName4 = getParam(p4,'tireName',name4);
%% 2) Create GUI
...
h.chk3 = uicontrol('Parent',h.panel,'Style','checkbox','Units','normalized', ...
    'Position',[0.1 0.86 0.8 0.04],'String',tireName3,'Value',1);
h.chk4 = uicontrol('Parent',h.panel,'Style','checkbox','Units','normalized', ...
    'Position',[0.1 0.82 0.8 0.04],'String',tireName4,'Value',1);
% axis for plot
h.ax = axes('Parent',h.fig,'Units','normalized','Position',[0.08 0.15 0.58 0.75]);

% edit fields
h.edFz  = createLabeledEdit(h.panel,[0.1 0.76 0.35 0.05],'Fz [N]:','1000');
h.edP   = createLabeledEdit(h.panel,[0.1 0.70 0.35 0.05],'Pressione [Pa]:',num2str(getParam(p1,'IP_NOM',1e5)));
h.edCam = createLabeledEdit(h.panel,[0.1 0.64 0.35 0.05],'Camber [deg]:','0');
h.edA   = createLabeledEdit(h.panel,[0.1 0.58 0.35 0.05],'Slip Angle \alpha [deg]:','0');
...
h.line1 = gobjects(0); h.line2 = gobjects(0); h.line3 = gobjects(0); h.line4 = gobjects(0);
...
updatePlot();
    ...
    function updatePlot(~,~)
        ...
        p1.userPressure = userP; p2.userPressure = userP; p3.userPressure = userP; p4.userPressure = userP;
        ...
        default1 = [0 0 1];  % blue
        default2 = [1 0 0];  % red
        default3 = [0 0.5 0];  % green
        default4 = [0 0 0];  % black
        if h.chk1.Value
        ...
        if h.chk3.Value
            Fy3 = arrayfun(@(a) MF_PAC2002_Fy_pure(0,a,userFz,gamma,p3),alphaRad);
            ln = plot(alphaDeg,Fy3,'-.','Color',default3,'LineWidth',1.5);
            h.line3(end+1) = ln;
        end
        if h.chk4.Value
            Fy4 = arrayfun(@(a) MF_PAC2002_Fy_pure(0,a,userFz,gamma,p4),alphaRad);
            ln = plot(alphaDeg,Fy4,':','Color',default4,'LineWidth',1.5);
            h.line4(end+1) = ln;
        end
    ...
    function updateLegend()
        ...
        if h.chk3.Value && ~isempty(h.line3)
            handles(end+1) = plot(h.ax,nan,nan,'k-.','LineWidth',1.5,'Tag','legendDummy'); %#ok<AGROW>
            entries{end+1} = tireName3; %#ok<AGROW>
        end
        if h.chk4.Value && ~isempty(h.line4)
            handles(end+1) = plot(h.ax,nan,nan,'k:','LineWidth',1.5,'Tag','legendDummy'); %#ok<AGROW>
            entries{end+1} = tireName4; %#ok<AGROW>
        end
