...
for iA = 1:numA
    ...
    for jK = 1:numK
        ...
        Fx_grid = zeros(numel(alpha_vec), numel(kappa_vec));
        Fy_grid = zeros(size(Fx_grid));
        for ii = 1:numel(alpha_vec)
            a = alpha_vec(ii);
            for jj = 1:numel(kappa_vec)
                k = kappa_vec(jj);
                Fx_p = MF_PAC2002_Fx_pure(k, 0, Fz, gamma, P, p);
                Fy_p = MF_PAC2002_Fy_pure(0, a, Fz, gamma, P, p);
                Gx   = MF_PAC2002_Gx(k, a, Fz, gamma, P, p);
                Gy   = MF_PAC2002_Gy(k, a, Fz, gamma, P, p);
                Fx_grid(ii,jj) = Fx_p * Gx;
                Fy_grid(ii,jj) = Fy_p * Gy;
            end
        end
...
function Gx = MF_PAC2002_Gx(kappa,alpha,Fz,gamma,P,p)
    ...
    Gx=cos(C_alpha*atan(B_alpha*alpha_s - E_alpha*(B_alpha*alpha_s - atan(B_alpha*alpha_s))))/cos(C_alpha*atan(B_alpha*rHx1-E_alpha*(B_alpha*rHx1-atan(B_alpha*rHx1))));
end
